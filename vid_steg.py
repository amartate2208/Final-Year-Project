...
label_l1 = tk.Label(root, text="___VIDEO STEGANOGRAPHY____", font=("Times New Roman", 35, 'bold'),
...
def msgtobinary(msg):#"Convert `data` to binary format as string"""
    if type(msg) == str:
        result= ''.join([ format(ord(i), "08b") for i in msg ])#takes all the elements of an iterable and joins them into a single string
...
def KSA(key):#Key-Generation Algorithm – A variable-length key 1 to 256 bytes is used to initialize a 256-byte state vector S, with elements S[0] to ...
    key_length = len(key)
    S=list(range(256)) 
    j=0
    for i in range(256):
...
def decryption(ciphertext):
    ...
    decoded=keystream^ciphertext#Keyword cipher is a form of monoalphabetic substitution. 
    dtext=''
    for c in decoded:
        dtext=dtext+chr(c)
        print(dtext)
...
def embed(frame):
    ...
    for i in frame:
        for pixel in i:
            r, g, b = msgtobinary(pixel)# convert RGB values to binary format
            if index_data < length_data:# modify the least significant bit only if there is still data to store
                pixel[0] = int(r[:-1] + binary_data[index_data], 2) # least significant red pixel bit
                index_data += 1
            if index_data < length_data:
                pixel[1] = int(g[:-1] + binary_data[index_data], 2) # least significant green pixel bit
                index_data += 1
            if index_data < length_data:
                pixel[2] = int(b[:-1] + binary_data[index_data], 2) # least significant blue pixel bit
                index_data += 1
            if index_data >= length_data:#if data is encoded, just break out of the loop
                break
        return frame    
...
def main():
    ...
    if Sel_F != 'mp4':
        ...
        ms.showinfo("Message", "Select Video .mp4 File!!!!!!")
...
def dec_main():
    cap = cv2.VideoCapture('stego.mp4') #decode 
    max_frame1=0;
    while(cap.isOpened()):
        ret, frame = cap.read()
        if ret == False:
